...
{
    public static readonly Error NotFoundError = new("Item not found");
    ...
    public void Error_Is_ReturnsTrueWhenErrorInChain()
    {
        ...
        Assert.True(outer.Is(NotFoundError));
        Assert.True(middle.Is(NotFoundError));
        Assert.True(inner.Is(NotFoundError));
    }
    ...
    {
        Assert.False(Error.None.Is(NotFoundError));
    ...
    public void Error_Is_WithPartialMessageMatch_ReturnsFalse()
    {
        ...
        Assert.False(err.Is(NotFoundError)); // Exact match only
    }
